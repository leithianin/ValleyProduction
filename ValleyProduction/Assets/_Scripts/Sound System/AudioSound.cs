...
public class AudioSound : ScriptableObject
{
    ...
    [SerializeField, Tooltip("The time before the sound loops. Put it less than zero make the sound not looping.")] private Vector2 loopTime = new Vector2(-1,-1);

    public AudioClip GetClip => clips[Random.Range(0, clips.Length)];
    public AudioMixerGroup Mixer => mixer;
    public float LoopTime => Random.Range(loopTime.x, loopTime.y);
}
